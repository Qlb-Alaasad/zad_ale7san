...
## New Tables
1. profiles — user profiles linked to auth.users, with role (admin/student) and status (pending/approved/rejected)
2. courses — academy courses with title, description, schedule
3. student_courses — join table linking students to courses
4. categories — evaluation categories with max_points
5. evaluations — student evaluations per category/week with points deducted
6. sessions — class/match/event sessions with start/end times
7. attendance — student attendance per session (present/late/absent)
8. financial_dues — student financial obligations (unpaid/paid)
9. tasks — student tasks with due dates and completion status
10. notifications — user notifications with type and read status
11. qr_tokens — attendance QR tokens linked to sessions with validity windows

## Security
- RLS enabled on all tables.
- Initial policies are restrictive: students access only their own rows, shared reference data is read-only to all.
- Admin policies will be added in a subsequent migration.
*/
